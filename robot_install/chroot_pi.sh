...
# the tests sd card as if it's the raspberry pi
...
# Write the tests image onto the sd card,
...
# (sudo) kpartx -v -a 2017-11-29-tests-stretch-lite.img
# then
...
# mount partition
mount -o rw ${1}2  /mnt/tests
mount -o rw ${1}1 /mnt/tests/boot

# mount binds
mount --bind /dev /mnt/tests/dev/
mount --bind /sys /mnt/tests/sys/
mount --bind /proc /mnt/tests/proc/
mount --bind /dev/pts /mnt/tests/dev/pts

# ld.so.preload fix
sed -i 's/^/#CHROOT /g' /mnt/tests/etc/ld.so.preload

# copy qemu binary
cp /usr/bin/qemu-arm-static /mnt/tests/usr/bin/
...
# unmount everything
umount /mnt/tests/{dev/pts,dev,sys,proc,boot,}
